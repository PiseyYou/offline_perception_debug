...
cv::Mat drawResultOptimized(cv::Mat &img_src, cv::Mat &img_lab,
  ...
  for (const auto &det : dect_src) {
    ...
    std::string label =
        "id_" + std::to_string(det.id) + ":" + cv::format("%.2f", det.score);
...
private:
  ...
  // 基于颜色判断生成label图 (纯RGB分量)
  // 白色/灰色 -> label=5 (RGB三通道接近，低色差)
  // 绿色 -> label=2 (G分量大于阈值且为主导通道)
  // 其他 -> label=3 (road)
  cv::Mat generateColorLabel(const cv::Mat &bgr_img) {
    cv::Mat label_map(bgr_img.rows, bgr_img.cols, CV_8UC1, cv::Scalar(3));

    // 对BGR图做均值滤波，基于邻域平均颜色判断
    cv::Mat blurred;
    ...
    for (int i = 0; i < blurred.rows; ++i) {
      ...
      for (int j = 0; j < blurred.cols; ++j) {
        ...
        int range = max_val - min_val; // RGB色差

        // Step 1: 白色/灰色检测 — RGB三通道接近(色差小)
        // 色差<35 且 亮度不太暗(均值>80) -> 灰白色 -> label=5
        if (range < 35 && (r + g + b) > 240) {
          label_ptr[j] = 5;
        }
        // Step 2: 绿色检测 — G分量为主导通道
        else if (g > 55 && g > r && g > b) {
          label_ptr[j] = 2;
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  if (config.infer_mode == 5) {
    ...
    config.finalPicDir = input_dir + "/cdt_sub_" + mode_suffix + "_0225_432_color/";
  }
  config.finalPcdDir =
      input_dir + "/pcd_" + mode_suffix + "_0225_432_color/"; // 设置最终PCD输出目录
